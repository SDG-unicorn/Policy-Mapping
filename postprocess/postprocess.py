...
def group_by_name_and_get_goaloverview(target_df, goal_df, sdg_references, grouping_factor='Policy'):
    #group target results by Policy and Goal and aggregate counts
    target_df = target_df.groupby([grouping_factor, 'Goal']).agg({'Count': ['sum']}).reset_index()
    #the above line creates tuples as column names so for append later to work columns need to be renamed
    target_df.columns = [grouping_factor, 'Goal', 'Count']
    #subset columns before append
    goal_df = goal_df[[grouping_factor, 'Goal', 'Count']]
    #append both results
    grouped_df = goal_df.append(target_df, ignore_index=True)
    #group again by Policy and Goal and aggregate counts for final df
    grouped_df = grouped_df.groupby([grouping_factor, 'Goal']).agg({'Count': ['sum']}).reset_index()
    ...
    grouped_df.columns = [grouping_factor, 'Goal', 'Count']
    grouped_df = sdg_references.join(grouped_df.set_index('Goal'), on='Goal', how='inner')
    #group by policy and sort by goal id
    grouped_df = grouped_df.groupby(grouping_factor).apply(lambda x: x.sort_values('goal_id'))
    return grouped_df
...
def create_json_files_for_bubbleplots(target_df, aggregated_goal_counts): #filename = None, output_path = None
    if aggregated_goal_counts.empty:
        final_dict = {'name': 'sdgs', 'children': [{"name": "No SDGs detected", "size": "100"}]}
        return final_dict
        ...
        final_dict = {'name': 'sdgs', 'children':
             [{"name": name, "size": value} for name, value in zip(labels, counts)]}
    else:
        target_df['Target'] = 'Target ' + target_df['Target'].astype(str)
        target_df.Count = target_df.Count.astype(int)
        aggregated_goal_counts.Count = aggregated_goal_counts.Count.astype(int)
        ...
        for i in range(0, len(goal_subset)):
            ...
            for row_dict in temp_target_df.to_dict(orient='records'):
                #print(f"\n{row_dict['name']}: {row_dict['size']}")
                total_target_count = total_target_count + row_dict['size']
